...
void Model::loadModel(std::string path)
{
    ...
    //Tell assimp that read the file, and if the model has other primitve shapes than triangles 
    //then convert all to triangles
    const aiScene *scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs);
    
    //After we load the model, we check if the scene and the root node of the scene are not null
    ...
    directory = path.substr(0, path.find_last_of("/")); //Model files located elsewhere but the project folder will not be loaded!
    processNode(scene->mRootNode, scene);
}

//RECURSIVE FUNCTION:
//Because each node (possibly) contains a set of children we want to first process the node in question
//and then continue processing all the node's children and so on
...
//helper function called to retrieve, load, and initialize the textures from the material. 
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
{
    ...
    //check the amount of textures stored in the material via its GetTextureCount
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        aiString str;
        mat->GetTexture(type, i, &str);
        bool skip = false;
        for(unsigned int j = 0; j < textures_loaded.size(); j++) // we store all the loaded textures in another vector declared at the top of the model's class file as a private variable
        {
